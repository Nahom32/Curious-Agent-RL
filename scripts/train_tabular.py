...
def ensure_training_log(log_path: str | Path = "training_tabular.log") -> logging.Handler | None:
    """Ensure tabular progress is written even when a parent configured logging.
    
    ``logging.basicConfig`` is a no-op when the unified runner has already
    installed a root handler. In that case the stream output still appears,
    but no file handler was created for the tabular log.
    """
